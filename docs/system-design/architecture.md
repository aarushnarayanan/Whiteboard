...
- `users.google_id` is now set via `GET /auth/google` → `GET /auth/google/callback` in `server/src/auth/routes.ts` (`findOrCreateGoogleUser`) — links to an existing password account by matching email, or creates a new account.
...
| GET | `/auth/google` | none | Redirects to Google's OAuth consent screen (`prompt=select_account` forces the account chooser rather than silently reusing a cached Google session), with a random `state` value in a short-lived cookie (CSRF guard). |
| GET | `/auth/google/callback` | none (state cookie + code) | Verifies `state`, exchanges the code for a token, fetches the Google profile, then `findOrCreateGoogleUser` links to an existing account by email or creates a new one, and signs the caller in. |
...
- **`auth/LoginForm.tsx`** — real email/password login and signup, plus forgot/reset-password, all as local `mode` states (`"login" | "signup" | "forgot" | "reset"`) in one component, calling `api/auth.ts`. A reset link's `?token=...` query param is read on mount (then stripped from the URL via `history.replaceState`) to drop straight into `"reset"` mode — there's no router, this is the app's only URL-driven state. "Continue with Google" navigates to `/auth/google` (real OAuth flow — see §4).
