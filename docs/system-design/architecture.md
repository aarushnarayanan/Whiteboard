...
- **`canvas/Canvas.tsx`** — a `react-konva` `Stage`/`Layer` bound to `useBoardDoc`'s `shapes` array. Owns pointer-driven shape drawing (rect/ellipse only), a Konva `Transformer` for resize (rotation disabled), a `captureThumbnail` imperative handle, and a self-contained zoom stepper (in/out/fit-to-screen) that manipulates the Konva stage's own scale/position directly — no server-side notion of zoom exists.
- **`canvas/Toolbar.tsx`** — the floating tool pill. Only `select`, and `rect`/`ellipse` (behind a "Shapes" flyout that also shows disabled Line/Star/Hexagon icons) are wired to the real `Tool` union; every other icon (Pen, Eraser, Arrow, Text, Sticky, Table, Frame, Image, Comment, Undo, Redo) is rendered `disabled`.
- **`canvas/types.ts`** — the entire shared vocabulary for canvas state: `Tool = "select" | "rect" | "ellipse"` and `ShapeObj = {id, type, x, y, width, height}`. No fill/stroke/text/rotation fields exist on `ShapeObj` today — shape color is hardcoded in `Canvas.tsx`'s render, not per-shape data.
...
- Text tool → `client/src/canvas/Canvas.tsx`, `Toolbar.tsx`, `types.ts` (`Tool` union + `ShapeObj`)
- Undo/redo (doesn't exist anywhere in the app) → `client/src/canvas/Canvas.tsx` (needs a new history stack), `Toolbar.tsx` (already has disabled buttons wired for this)
